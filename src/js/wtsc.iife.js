var wtsc = (function (exports) {
    ...
    function uniteClassSelectors(ClassSelectors, className) {
        return `${ClassSelectors}.${hypnenate(className)}`;
    }
    function uniteClassNames(classNames, className) {
        return `${classNames} ${hypnenate(className)}`.trim();
    }
    ...
    const beforeConfigUndate = defEventKey();
    const configUndated = defEventKey();
    ...
    function setConfig(_config) {
        ...
        Object.keys(_config).forEach((key) => (config[key] = _config[key]));
    ...
    function defUseStatePackager(useState) {
        exports.pack = (value, pack) => {
            if (pack) {
                pack[1](value);
                return pack;
...
本css将会被忽略，请查看您是否有强制类型转换来解决问题`);
    ...
    exports.defUseStatePackager = defUseStatePackager;
